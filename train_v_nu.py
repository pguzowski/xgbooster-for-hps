...
params = params_getter(args.model, 'nu' if args.split_ext_nu else None)
getit = params.getit
featmap = params.featmap
...
data_nu = getit(tn)
data_nu_train = data_nu[:int(len(data_nu)*.7)]
...
labels = [1]*len(data_sig_train) + [0]*len(data_nu_train)

#scale = float(len(data_sig_train))/float(len(data_cos_train))
#weights = [1]*len(data_sig_train) + [scale]*len(data_cos_train)
#weights = [1]*len(data_sig_train) + [scale]*len(data_cos_train)

print('len train sig',len(data_sig_train),'nu',len(data_nu_train))
print('len test sig',len(data_sig_test),'nu',len(data_nu_test),'cos',len(data_cos_test))

#print(xx)

xgb_train = xgboost.DMatrix(pandas.concat([data_sig_train, data_nu_train]), label=labels)
...
watchlist = [(xgb_train, 'train'), (xgb_test_sig, 'test_sig'),(xgb_test_nu,'test_nu'),(xgb_test_ext,'test_ext')]
param = params.param
num_round = args.num_round

bdt = xgboost.train(param, xgb_train, num_round, watchlist)

bdt.save_model('models/%s/nu_model_r%d.json'%(params.name,r13.num))

bdt.dump_model('models/%s/nu_dump_r%d.raw.txt'%(params.name,r13.num),featmap)
